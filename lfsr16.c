/* lfsr16.c - Template Test of 16 bit LFSR  Version 0.1.0 */
...
/* lfsr_table.pdf                                       */
/* 16 bit lfsr is 16 14 13 11 relative to 1             */
/* reversed, it is 5 3 2 0 relative to zero             */
/********************************************************/

#define MYLFSROUT (xx->out = (((xx->lfsr >> 5) \
   ^ (xx->lfsr >> 3) \
   ^ (xx->lfsr >> 2) \
   ^ (xx->lfsr >> 0)) & 1))

#define MYLFSRROLL (xx->lfsr = ((xx->lfsr >> 1) \
   | (xx->out << 15)) & 0xffff)
...
int main(void)
   ...
   xx->lfsr = gsl_rng_get(xx->rng) & 0xffff;
   gsl_rng_free(xx->rng);
   /****************************************************/
