/* randu.c - Template Test of 32 bit RANDU  Version 0.1.0 */
...
/* This program runs a template test on a 32 bit RANDU random        */
/* number generator.                                                 */
/* The expected result from this test is a P-value greater           */
/* than 0.999 in more than 5% of the tests.                          */
/* The number of samples is less than the period length of           */
...
int main(void)
   ...
   /* values in the randu seed                            */
   xx->rng = (gsl_rng *) gsl_rng_alloc(gsl_rng_taus);
   gsl_rng_set(xx->rng, xx->dttk);
   xx->seed  = gsl_rng_get(xx->rng) | 1;
   // xx->seed = (dttk & 0xffff0000) | 0x00010000;
   gsl_rng_free(xx->rng);
   /****************************************************/
